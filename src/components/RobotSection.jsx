...
function buildRobot(mat) {
  ...
  for (let i = -2; i <= 2; i++) {
    const mt = box(0.013, 0.025, 0.015, mat.acc); mt.position.set(i * 0.024, -0.099, 0.143); head.add(mt)
  }
  ...
  for (let y of [1.09, 1.20, 1.31, 1.42]) {
  ...
  for (let i = 0; i < 7; i++) {
    const sp2 = box(0.073, 0.072, 0.023, i % 2 === 0 ? mat.dark : mat.joint)
    sp2.position.set(0, 1.41 - i * 0.102, -0.154)
    torso.add(sp2)
  }
  ...
  function mkLeg(side) {
    ...
    footGroup.position.set(s * 0.013, -0.400, 0.030)
...
function drawPanel(p) {
  ...
  for (let y = 94; y <= 94; y += 6) {
    ctx.strokeStyle = 'rgba(40,70,180,0.22)'; ctx.lineWidth = 0.5; ctx.beginPath(); ctx.moveTo(4, y); ctx.lineTo(252, y); ctx.stroke()
  }
...
export default function RobotSection() {
  ...
  useEffect(() => {
    ...
    const botX = isMobile ? 1.3 : 2.4
    const panX = isMobile ? 1.9 : 3.6
    const camZ = isMobile ? 7.5 : 5.9

    const renderer = new THREE.WebGLRenderer({ antialias: true, alpha: true, powerPreference: 'high-performance' })

    // Use container dimensions and pass `false` as 3rd argument to prevent inline width/height overriding CSS
    ...
    camera.position.set(0, 0.28, camZ); camera.lookAt(0, 1.26, 0)
    ...
    const updateRain = () => {
      rFrame++; if (rFrame % 4 !== 0) return
      rCtx.fillStyle = 'rgba(0,0,6,0.08)'
    ...
    panL.mesh.position.set(-panX, 1.44, 0.8); panL.mesh.rotation.y = 0.42; scene.add(panL.mesh)
    panR.mesh.position.set(panX, 1.44, 0.8); panR.mesh.rotation.y = -0.42; scene.add(panR.mesh)
    ...
    const restPose = (bot) => {
      bot.rArm.uP.rotation.set(0.06, 0, 0.18)
      bot.rArm.fP.rotation.set(0.12, 0, 0)
      bot.lArm.uP.rotation.set(0.06, 0, -0.18)
      bot.lArm.fP.rotation.set(0.12, 0, 0)
    }
    ...
    const animate = () => {
      ...
      if (!isMobile) {
        ...
        if (overlayRef.current) overlayRef.current.style.transform = 'translate(-50%, -50%)'
      }
  ...
  // Auto scroll trigger on clicking the scroll hint element
  ...
  return (
    <section className="robot-section" id="robots">
      <div
        ...
        role="img"
        aria-label="Two AI robots welcoming visitors with animated holographic displays"
      />
